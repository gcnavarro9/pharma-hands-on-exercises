...
mean(y)
#> [1] 2.5
Created on 2025-12-28 with reprex v2.1.0
```
...
Example <- tribble(~SAFFL, ~PARAM, ~ADY, ~AVAL,
  "Y", "Bicarbonate (MEQ/L)", 1, 75,
  "Y", "Bicarbonate (MEQ/L)", 2, 80,
  "Y", "Bicarbonate (MEQ/L)", 5, 85,
  "Y", "Bicarbonate (MEQ/L)", 7, 100
)

Sample_Output <- Example %>%  
    filter(SAFFL == "Y") %>% 
    filter(PARAM == "Bicarbonate (MEQ/L)")


ggplot(data = Sample_Output,
       mapping = aes(x = ADY, y = AVAL)) +
  geom_point(aes(colour = "THERAPY1"))
```