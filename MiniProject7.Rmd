...
adsl_counts <- function(adsl_FilePath){
    adsl_saf <- read_xpt(adsl_FilePath) %>%
    filter(SAFFL == "Y")
...
adsl_counts <- function(adsl_filepath) {
  ...
  Big_N_cnt <-  adsl_filepath   %>%
    group_by( TRT01AN, TRT01A  ) %>%
    count(name = "N")
  Big_N_cnt
  
  small_n_cnt <-  adsl_filepath  %>%
    group_by( TRT01AN, TRT01A,  SEX ) %>%
    count(name = "n")
  small_n_cnt
...
library(cowplot)
library(ggdist)
create_plot <- function(myData, xlab, ylab){
  ...
  ggplot(mapping = aes(x = LBSTRESN, 
                       y = as.factor(WEEK))) +
  coord_cartesian(xlim=c(0, 150)) +
  stat_pointinterval() +
  geom_point(data=filter(ALT, LBNRIND %in% c('LOW', 'HIGH') & WEEK %in% c(0, 1, 2, 4, 8, 16)), color="red") +
  labs(x = xlab, y = ylab)

ggdraw(plot_1) + 
  draw_label("DRAFT", color = "grey", alpha=0.3, size = 90, angle = 45) 
  
}

create_plot(myData = "https://github.com/phuse-org/phuse-scripts/raw/master/data/adam/cdisc/adlbc.xpt", xlab = "Alanine Aminotransferase U/L", ylab= "Week")
