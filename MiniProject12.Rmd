...
adsl_data <- import("https://github.com/phuse-org/phuse-scripts/raw/master/data/adam/cdisc/adsl.xpt")
```
...
Big_N_cnt<-calculate_Big_N_cnt(wrangle_data(adsl_data))
calculate_Big_N_cnt(wrangle_data(adsl_data))
```
...
small_n_cnt<-calculate_small_n_cnt(wrangle_data(adsl_data))
calculate_small_n_cnt(wrangle_data(adsl_data))
```
...
test_that("calculate_small_n_cnt() output has expected dim, columns, and types", {
   ...
   expect_equal(dim(calculate_small_n_cnt(test_data)),
                c(length(unique(c(calculate_small_n_cnt(test_data)$TRT01A,calculate_small_n_cnt(test_data)$SEX))), 4))
})
...
test_that("adsl_counts() produces correct output", {
   
  expect_named(adsl_test_data, c("TRT01A", "SEX", "perc"))
  expect_equal(dim(adsl_test_data), c(4, 3))
  ...
  expect_identical(aggregate_perc$x, c(100,100))
})
```
