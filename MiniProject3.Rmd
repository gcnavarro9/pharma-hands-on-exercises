...
varSummary <- function(.data = NULL, variable = NULL) {
  .data %>%
    group_by(TRT01AN,TRT01A,SEX) %>%
    summarize(mean = mean({{variable}}, na.rm = TRUE) %>% round(digits = 1) %>% format(nsmall=1)  ,
              sd = sd({{variable}}, na.rm = TRUE) %>% round(digits = 1) %>% format(nsmall = 1), 
...
weight_stat<-adsl_eff %>%
  varSummary(WEIGHTBL)
height_stat<-adsl_eff %>%
  varSummary(HEIGHTBL)
