...
ALT2 %>%
  ggplot() +
  geom_bar(mapping = aes(x = as.factor(WEEK), fill=LBNRIND ))
```
...
plot_1 <- ALT2 %>% 
  ...
  stat_pointinterval() +
  geom_point(data=filter(ALT2, LBNRIND %in% c('LOW', 'HIGH') & WEEK %in% c(0, 1, 2, 4, 8, 16)), color="red") +
  labs(x = 'Alanine Aminotransferase U/L', y = 'Week')
