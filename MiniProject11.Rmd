...
get_cat_demo <- function(.data = adsl_saf, variable = RACE) {
  ...
  var_char <- rlang::as_name(substitute(variable))# complete this line here to solve this problem
  ...
  Age_trans <- pivot_wider(age_n_pct,
                           ...
                           values_fill = "0",
                           names_sep = "_")
  age_cat <- rename(Age_trans, category=var_char)
  sorted_age_cat <- Age_trans %>%
    #arrange(category)
  return(Age_trans)
}
...
```
```{r}
adsl_counts <- function(dat, var, ...) {
  
  var_char <- rlang::as_name(substitute(var))# complete this line here to solve this problem
...
table1 <- import(inFile) %>%
    filter(SAFFL == "Y") %>%
  adsl_counts(var = SEX, digits = 1)
table1
```