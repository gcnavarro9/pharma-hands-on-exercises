...
## Create function
demog_sum <- function(data, var) {
  var_sym <- sym(var) # Convert string to symbol for tidy evaluation
  ...
  calc_cnts <- function(var_sym) {
    small_n_cnt <- data %>%
      group_by(TRT01AN, TRT01A, !!var_sym) %>%
      count(name = "n")
    adsl_mrg_cnt <- small_n_cnt %>%
      left_join(bigncnt, by = c("TRT01A", "TRT01AN")) %>%
  ...
  # Summarize the table for each category
  summary_counts <- calc_cnts(var_sym) %>%
    select(TRT01A, !!var_sym, npct) %>%
    pivot_wider(names_from = TRT01A, values_from = npct)
  return(summary_counts)
}
 
# Calculate summary by SEX
sex_summary <- demog_sum(adsl_saf, "SEX")
print(sex_summary)
 
# Calculate summary by AGEGR1
age_summary <- demog_sum(adsl_saf, "AGEGR1")
print(age_summary)
 
# Calculate summary by RACE
race_summary <- demog_sum(adsl_saf, "RACE")
print(race_summary)
```
