...
small_n_cnt2 <- adsl_eff %>%
  group_by(TRT01AN, TRT01A, SEX, AGEGR1N, AGEGR1) %>%
  count(name = "n")

adsl_mrg_cnt2 <- small_n_cnt2 %>%
  left_join(Big_N_cnt2)%>%
  mutate(perc = round(n/N*100, digits=1)) %>% 
  mutate(perc_char = format(perc, nsmall=1)) %>% 
  mutate(npct = paste(n, "(", perc_char, ")" ) ) %>%
  mutate(SEX = recode(SEX, "M" = "Male", "F" = "Female"))%>%
  ungroup() %>% select(TRT01A, AGEGR1, SEX, npct)%>%
  arrange(AGEGR1)

adsl_mrg_cnt2

adsl_mrg_cnt2_final <- adsl_mrg_cnt2 %>%
  pivot_wider(names_from = TRT01A, values_from = npct)
...
small_n_cnt3 <- adae_saf %>%
  group_by(TRTAN, TRTA, AEBODSYS) %>%
  count(name = "n")

adae_mrg_cnt3 <- small_n_cnt3 %>%
  left_join(Big_N_cnt3)%>%
  mutate(perc = round(n/N*100, digits=1)) %>% 
  mutate(perc_char = format(perc, nsmall=1)) %>% 
  mutate(npct = paste(n, "(", perc_char, ")" ) ) %>%
  ungroup() %>% select(TRTA, AEBODSYS, npct)%>%
  arrange(AEBODSYS)

adae_mrg_cnt3

adae_mrg_cnt3_final <- adae_mrg_cnt3 %>%
  pivot_wider(names_from = TRTA, values_from = npct)

names(adae_mrg_cnt3_final)[names(adae_mrg_cnt3_final) == "AEBODSYS"] <- "Body System or Organ Class"
adae_mrg_cnt3_final
```
