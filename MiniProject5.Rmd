...
```{r}
vs_s <- import("https://github.com/phuse-org/phuse-scripts/raw/master/data/adam/cdiscpilot01/advs.xpt")
...
xLab1 <- quo_name(plot_pulse$mapping$x)
yLab1 <- quo_name(plot_pulse$mapping$y)
groupLab1 <- quo_name(plot_pulse$mapping$group)

study1 <- uniqueVal(plot_pulse$data$STUDYID)
measure1 <- uniqueVal(plot_pulse$data$PARAM)

xLab <- ifelse(xLab == "ADY", "Day within study period", xLab1)
groupLab <- case_when(groupLab1 == "USUBJID" ~ "Subject", 
                      groupLab1 == "TRTA" ~ "Treatment",
                      TRUE ~ groupLab)

plot_pulse + 
  labs(title = glue::glue("Study {study1}"),
       subtitle = glue::glue("Plot of {measure1} by {groupLab}"),
       x = xLab1,
       y = glue::glue("{measure1}"))
```
